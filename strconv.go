...
func stringToValue(s string, v reflect.Value) os.Error {
    if !v.CanSet() {
        return os.NewError("Value is not assignable")
    }
    ...
    case reflect.Int64:
        ...
        v.SetInt(x)
        if v.OverflowInt(x) {
            err = os.NewError("Assignment precision error")
        }
    case reflect.Uint64:
...
func stringToWideType(s string, t reflect.Type) (v reflect.Value, err os.Error) {
    handle := func (i interface{}, e os.Error) {
        v = reflect.ValueOf(i)
        err = e
    }
    switch k := t.Kind(); k {
